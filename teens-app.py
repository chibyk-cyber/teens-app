...
<style>
.stApp {{
    background-image: url("https://imgur.com/a/OuRIfwF");
...
def get_conn():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS schedule (id INTEGER PRIMARY KEY, activity TEXT, date TEXT, created_at TEXT)")
    return conn

# ================== Social Media ==================
def ui_social_media():
    st.subheader("🌐 Social Media")
    st.write("Quick links to your favorite platforms:")
...
def ui_scheduler():
    ...
    st.caption("Keep track of your important tasks and deadlines.")

    with st.form("add_activity", clear_on_submit=True):
        activity = st.text_input("Activity", placeholder="Read Chapter 5 - Physics")
        date = st.date_input("Date")
        if st.form_submit_button("Add Activity") and activity:
            with get_conn() as conn:
                conn.execute(
                    "INSERT INTO schedule (activity, date, created_at) VALUES (?, ?, ?)",
                    (activity.strip(), str(date), datetime.utcnow().isoformat())
                )
                conn.commit()
            st.success("Activity added ✅")

    with get_conn() as conn:
        rows = conn.execute("SELECT id, activity, date FROM schedule ORDER BY date").fetchall()

    if rows:
        st.markdown("### Upcoming Activities")
        df = pd.DataFrame(rows, columns=["ID", "Activity", "Date"])
        st.table(df[["Activity", "Date"]])
    else:
        st.info("No activities yet. Add one above ✍️")

# ================== Exam Questions ==================
questions = {
    "Math": [
        {"q": "What is 12 × 8?", "a": "96"},
        {"q": "Simplify: 2x + 3x", "a": "5x"},
        {"q": "Solve: 15 ÷ 3", "a": "5"},
    ],
    "Science": [
...
# ================== Navigation ==================
PAGES = {
    "Social Media": ui_social_media,
    "Schedule": ui_scheduler,
    "Exam Questions": ui_exam_questions,
